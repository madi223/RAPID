...
logdir="standard-25-scal/"
trace=$logdir"scen"$scen"-All."$buff"."$rtt".csv"
WORK="/home/mdiarra/RAPID/ns3-mmwave/"
#WORK=$(pwd)
cd $WORK
...
comm=" ./waf --run \"standard_scal --simTime="$simTime" --data="$data" --stream="$stream" --buff="$buff" --scen="$scen" --serverDelay="$rtt" --run="
