...
if [ $# -lt 7 ]
then
   echo "[usage]: ./start-nlos.sh <scen>  <run1,runn> <simTime> <data> <stream> <buff> <rtt>"
   exit 1
fi
...
comm=" ./waf --run \"standard200Mhz_5g --simTime="$simTime" --data="$data" --stream="$stream" --buff="$buff" --scen="$scen" --nice=true --serverDelay="$rtt" --run="
