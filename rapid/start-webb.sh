...
logdir="web-proxy-bcubic/"
trace=$logdir""$scen"-"$cca"-All."$buff"."$rtt".csv"
WORK="/home/mdiarra/bbr/proxy/webbc/"
cd $WORK
...
comm=" ./waf --run \"webb_5g --simTime="$simTime" --bbr=true --data="$data" --stream="$stream" --buff="$buff" --serverDelay="$rtt" --run="
