...
std::ofstream tcplog ("clean-tcp.csv",std::ios::out | std::ios::app);
TypeId
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
 ...
 // MELD log
 tcplog << Simulator::Now ().GetSeconds () << "," << m_tcb->m_cWnd.Get ()  << "," << ((Time)(m_tcb->m_lastRtt)).GetMilliSeconds () << "," <<(m_tcb->m_bytesInFlight*8)/(((Time)(m_tcb->m_lastR\
tt)).GetSeconds()*1000000) << "," <<(double(0)*8)/(0.000125*1000000) << "," << 0 << "," <<(uint32_t)(m_tcb->m_bytesInFlight) <<","<< m_tcb->m_congState <<","<< m_endPoint->\
GetPeerPort () <<","<<m_endPoint->GetLocalPort() << std::endl;
 // end MELD log 
  return sz;
}
...
void TcpSocketBase::UpdateWindowSize (uint32_t size)
{
  m_rWnd = size;
  m_tcb->m_rw = size;
  use_rcv = false;
